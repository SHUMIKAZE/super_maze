...
class Raycaster:
    ...
    def cast_single_ray(self, ray_index, player_x, player_y, player_angle):
        relative_angle = (ray_index / self.RAY_COUNT) * self.FIELD_OF_VIEW - (self.FIELD_OF_VIEW / 2)
        ...
        while distance < self.MAX_DISTANCE:
            ...
            if row < 0 or row >= len(game_maze.map) or col < 0 or col >= len(game_maze.map[0].strip()):
...
while True:
    ...
    if drawing == 1:
        if (int(new_y) < 0 or int(new_y) >= len(game_maze.map) or 
            int(new_x) < 0 or int(new_x) >= len(game_maze.map[0].strip())):
            break 
